...
static P384_JWK_FIXTURE: &str = r#"{
        ...
        "crv": "P-384",
        "kid": "sig-1650284838",
        "x": "3VdrPTk7esh1TmQqE0LNswQVY1kGCD0dnxhaPc6Ei9nXSCUQf46lLUoXg00zWYPM",
        "y": "z3P8BMHgDX9wiVkM-cTU0zj_N8uijd1hPhzXC4TSf-FgeMvP7QUqEYImAoEaGMkZ",
        "alg": "ES384"
    }"#;

static RSA256_JWK_FIXTURE: &str = r#"{
...
static RSA384_JWK_FIXTURE: &str = r#"{
        "p": "6AQ4yHef17an_i5LQPHNIxzpH65xWOSf_qCB7q-lXyM",
        "kty": "RSA",
        "alg": "RS384",
        "q": "tSVfpefCsf1iWmAs1zYvxdEsUiv0VMEuQBtbTijj_OE",
...
static RSA512_JWK_FIXTURE: &str = r#"{
        "p": "6AQ4yHef17an_i5LQPHNIxzpH65xWOSf_qCB7q-lXyM",
        "kty": "RSA",
        "alg": "RS512",
        "q": "tSVfpefCsf1iWmAs1zYvxdEsUiv0VMEuQBtbTijj_OE",
...
fn serialize_hs384() {
    ...
    assert_eq!(
        jwk.to_string(),
        r#"{"kty":"oct","k":"KioqKioqKioqKioqKioqKg","alg":"HS384"}"#
    );
}

#[test]
fn deserialize_hs512() {
    let jwk_str = r#"{
        "kty": "oct",
        "k": "tAON6Q",
        "alg": "HS512",
...
fn serialize_hs512() {
    ...
    assert_eq!(
        jwk.to_string(),
        r#"{"kty":"oct","k":"KioqKioqKioqKioqKioqKg","alg":"HS512"}"#
    );
}
...
#[test]
fn deserialize_rs384() {
    let jwk = JsonWebKey::from_str(RSA384_JWK_FIXTURE).unwrap();
    assert_eq!(
        jwk,
        JsonWebKey {
            key: Box::new(Key::RSA {
            ...
            algorithm: Some(Algorithm::RS384),
            key_id: None,
            key_ops: KeyOps::WRAP_KEY,
            key_use: Some(KeyUse::Encryption),
            x5: Default::default(),
...
#[test]
fn deserialize_rs512() {
    let jwk = JsonWebKey::from_str(RSA512_JWK_FIXTURE).unwrap();
    assert_eq!(
        jwk,
        JsonWebKey {
            key: Box::new(Key::RSA {
            ...
            algorithm: Some(Algorithm::RS512),
            key_id: None,
            key_ops: KeyOps::WRAP_KEY,
            key_use: Some(KeyUse::Encryption),
            x5: Default::default(),
...
#[cfg(feature = "pkcs-convert")]
#[test]
fn p384_private_to_pem() {
    let jwk = JsonWebKey::from_str(P384_JWK_FIXTURE).unwrap();
    #[rustfmt::skip]
    assert_eq!(
        jwk.key.to_pem(),
"-----BEGIN PRIVATE KEY-----
MIG2AgEAMBAGByqGSM49AgEGBSuBBAAiBIGeMIGbAgEBBDCrOnzYH134I6AJ82n6
7fc9gByCCSCgLmfdSfFz7KGJrAp2sNJXDrk3CqCxxADE0jehZANiAATdV2s9OTt6
yHVOZCoTQs2zBBVjWQYIPR2fGFo9zoSL2ddIJRB/jqUtSheDTTNZg8zPc/wEweAN
f3CJWQz5xNTTOP83y6KN3WE+HNcLhNJ/4WB4y8/tBSoRgiYCgRoYyRk=
-----END PRIVATE KEY-----
"
    );
}
...
#[cfg(feature = "pkcs-convert")]
#[test]
fn p384_public_to_pem() {
    let jwk = JsonWebKey::from_str(P384_JWK_FIXTURE).unwrap();
    #[rustfmt::skip]
    assert_eq!(
        jwk.key.to_public().unwrap().to_pem(),
"-----BEGIN PUBLIC KEY-----
MHYwEAYHKoZIzj0CAQYFK4EEACIDYgAE3VdrPTk7esh1TmQqE0LNswQVY1kGCD0d
nxhaPc6Ei9nXSCUQf46lLUoXg00zWYPMz3P8BMHgDX9wiVkM+cTU0zj/N8uijd1h
...
#[cfg(feature = "pkcs-convert")]
#[test]
fn rs256_private_to_encoding_key_via_pem() {
    let jwk = JsonWebKey::from_str(RSA256_JWK_FIXTURE).unwrap();
...
#[cfg(feature = "pkcs-convert")]
#[test]
fn rs256_public_to_decoding_key_via_pem() {
    let jwk = JsonWebKey::from_str(RSA256_JWK_FIXTURE).unwrap();
    let pem = jwk.key.to_public().unwrap().to_pem();
    jsonwebtoken::DecodingKey::from_rsa_pem(pem.as_bytes()).unwrap();
}

#[cfg(feature = "pkcs-convert")]
#[test]
fn rs384_private_to_encoding_key_via_pem() {
    let jwk = JsonWebKey::from_str(RSA256_JWK_FIXTURE).unwrap();
    let pem = jwk.key.to_pem();
...
#[cfg(feature = "pkcs-convert")]
#[test]
fn rs384_public_to_decoding_key_via_pem() {
    let jwk = JsonWebKey::from_str(RSA256_JWK_FIXTURE).unwrap();
    let pem = jwk.key.to_public().unwrap().to_pem();
    jsonwebtoken::DecodingKey::from_rsa_pem(pem.as_bytes()).unwrap();
}
...
#[cfg(feature = "pkcs-convert")]
#[test]
fn rs512_private_to_encoding_key_via_pem() {
    let jwk = JsonWebKey::from_str(RSA256_JWK_FIXTURE).unwrap();
    let pem = jwk.key.to_pem();
...
#[cfg(feature = "pkcs-convert")]
#[test]
fn rs512_public_to_decoding_key_via_pem() {
    let jwk = JsonWebKey::from_str(RSA256_JWK_FIXTURE).unwrap();
    let pem = jwk.key.to_public().unwrap().to_pem();
    jsonwebtoken::DecodingKey::from_rsa_pem(pem.as_bytes()).unwrap();
}
...
#[test]
fn rsa_is_private() {
    let private_jwk = JsonWebKey::from_str(RSA256_JWK_FIXTURE).unwrap();
    assert!(private_jwk.key.is_private());
    assert!(!private_jwk.key.to_public().unwrap().is_private());
